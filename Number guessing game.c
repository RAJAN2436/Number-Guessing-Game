...
void play_game(int *high_score) {
    ...
    while (attempts < max_attempts) {
        ...
        printf("\nAttempt %d of %d — Enter your guess: ", attempts, max_attempts);
        if (scanf("%d", &guess) != 1) {
            clear_input_buffer();
        ...
        if (guess > secret) {
            ...
            printf(C_GREEN C_BOLD "\n🎉 Correct! You guessed the number %d in %d attempt(s).\n" C_RESET,
                   secret, attempts);
            won = 1;
            break;
        }
    ...
    if (!won) {
        printf(C_RED C_BOLD "\n☹️ You've used all attempts. The number was %d.\n" C_RESET, secret);
    }
    ...
    while (running) {
        ...
        switch (menu_choice) {
            ...
            case 3:
                ...
                break;
            case 4:
                printf(C_MAG "Thanks for playing — Goodbye!\n" C_RESET);
                running = 0;
                break;
            default:
